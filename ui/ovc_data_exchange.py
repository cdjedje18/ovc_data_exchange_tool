...
from common.modules.ovc_data_exchange import family_exchange, beneficiary_exchange
...
def create_ovc_data_exchange_frame(parent, show_frame):
    frame = ctk.CTkFrame(parent)

    # ── Header ──────────────────────────────────────────────────────────────
    header = ctk.CTkFrame(frame)
    header.pack(fill="x", padx=15, pady=12)
    ctk.CTkLabel(header, text="OVC Data Exchange", font=ctk.CTkFont(size=20, weight="bold")).pack(side="left")
    ...
    selected_family_program = ctk.StringVar(value=programs[0] if programs else "")
    family_program_dropdown = ctk.CTkOptionMenu(form_frame, values=programs, variable=selected_family_program, width=220)
    family_program_dropdown.grid(row=0, column=1, pady=4, sticky="ew")

    def refresh_programs():
        options = load_program_options()
        _set_option_menu_values(family_program_dropdown, selected_family_program, options)
    ...
    ctk.CTkLabel(form_frame, text="Beneficiary Program:").grid(row=1, column=0, sticky="w", pady=4)
    ...
    beneficiary_program_dropdown.grid(row=1, column=1, pady=4, sticky="ew")
    ctk.CTkButton(form_frame, text="↻", width=36, command=refresh_programs).grid(row=1, column=2, padx=(6, 0), pady=4)
    ...
    def load_family_attributes(_=None):
        ...
        if not program:
            family_attributes_list = []
            _set_option_menu_values(family_waiver_dropdown, selected_family_waiver, ["<select a program first>"])
            return
        try:
    ...
    # ── Row 2 – Family Waiver Attribute ──────────────────────────────────────
    ctk.CTkLabel(form_frame, text="Family Waiver Attr:").grid(row=2, column=0, sticky="w", pady=4)
    ...
    family_waiver_dropdown.grid(row=2, column=1, pady=4, sticky="ew")
    ctk.CTkButton(form_frame, text="↻", width=36, command=load_family_attributes).grid(row=2, column=2, padx=(6, 0), pady=4)
    ...
    # ── Row 11 – Run Family button ───────────────────────────────────────────
    run_family_button = ctk.CTkButton(form_frame, text="Run Family Exchange", width=220)
    run_family_button.grid(row=11, column=0, columnspan=2, pady=(12, 4))

    # ── Row 12 – Cancel Family button ────────────────────────────────────────
    cancel_family_button = ctk.CTkButton(form_frame, text="Cancel Family", width=220, state="disabled")
    cancel_family_button.grid(row=12, column=0, columnspan=2, pady=(0, 8))

    # ── Row 13 – Run Beneficiary button ──────────────────────────────────────
    run_beneficiary_button = ctk.CTkButton(form_frame, text="Run Beneficiary Exchange", width=220)
    run_beneficiary_button.grid(row=13, column=0, columnspan=2, pady=(4, 4))

    # ── Row 14 – Cancel Beneficiary button ───────────────────────────────────
    cancel_beneficiary_button = ctk.CTkButton(form_frame, text="Cancel Beneficiary", width=220, state="disabled")
    cancel_beneficiary_button.grid(row=14, column=0, columnspan=2, pady=(0, 8))

    # ── Row 15 – Clear log button ─────────────────────────────────────────────
    ctk.CTkButton(
        form_frame, text="Clear Prints", width=220,
        command=lambda: log_text.delete("1.0", "end"),
    ).grid(row=15, column=0, columnspan=2, pady=(0, 15))
    ...
    beneficiary_thread = None
    beneficiary_cancel_event = None

    def process_queue():
        nonlocal family_thread, family_cancel_event, beneficiary_thread, beneficiary_cancel_event
        try:
            while True:
        ...
        if family_thread is not None and not family_thread.is_alive():
            run_family_button.configure(state="normal")
            cancel_family_button.configure(state="disabled")
            family_thread = None
            family_cancel_event = None

        if beneficiary_thread is not None and not beneficiary_thread.is_alive():
    ...
    def _build_execution_config():
        ...
        if not family_program:
            append_log("[ERROR] Family program must be selected and loaded correctly")
            return None
        ...
        if not family_waiver_id:
            append_log("[ERROR] Family waiver attribute must be selected")
            return None
    ...
    def run_family_exchange():
        ...
        append_log(
            f"[INFO] Starting Family Exchange | family='{execution_config.family_program['name']}' "
            f"| orgunits={len(execution_config.orgunits)} | pageSize={execution_config.page_size} "
            f"| async={execution_config.async_import}"
        )

        run_family_button.configure(state="disabled")
        cancel_family_button.configure(state="normal")
        family_cancel_event = threading.Event()

        family_thread = threading.Thread(
            target=_make_worker(
                family_exchange.execute,
                execution_config,
                family_cancel_event,
                "Family Exchange",
    ...
    def cancel_family_exchange():
        if family_cancel_event:
            ...
            append_log("[INFO] Cancellation requested for Family Exchange.")

    run_family_button.configure(command=run_family_exchange)
    cancel_family_button.configure(command=cancel_family_exchange)

    # ── Run Beneficiary ──────────────────────────────────────────────────────
    def run_beneficiary_exchange():
        nonlocal beneficiary_thread, beneficiary_cancel_event

        if beneficiary_thread is not None and beneficiary_thread.is_alive():
            append_log("[WARN] Beneficiary exchange is already running.")
            return
        ...
        append_log(
            f"[INFO] Starting Beneficiary Exchange | beneficiary='{execution_config.beneficiary_program['name']}' "
            f"| orgunits={len(execution_config.orgunits)} | pageSize={execution_config.page_size} "
            f"| async={execution_config.async_import}"
        )
        ...
        beneficiary_cancel_event = threading.Event()

        beneficiary_thread = threading.Thread(
            target=_make_worker(
                beneficiary_exchange.execute,
                execution_config,
                beneficiary_cancel_event,
                "Beneficiary Exchange",
                pass_cancel_event=True,
            ),
            daemon=True,
        )
        beneficiary_thread.start()

    def cancel_beneficiary_exchange():
        if beneficiary_cancel_event:
            beneficiary_cancel_event.set()
            append_log("[INFO] Cancellation requested for Beneficiary Exchange.")

    run_beneficiary_button.configure(command=run_beneficiary_exchange)
    cancel_beneficiary_button.configure(command=cancel_beneficiary_exchange)
